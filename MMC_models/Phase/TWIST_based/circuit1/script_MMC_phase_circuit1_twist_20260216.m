...
latest_MMC_parameters = "param_phase_circuit1_20260216.m";
latest_twist_parameters = "twist_parameters.m";

sim_time = 0.6; %overall simulation time
...
time_decharge = 0.6; % [s] Decharging time
...
VDC_name = string(VDC);
    if(is6VExternallySupplied == false)
        if(isC1low1Active == true)
            save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_feeder_avecC1low_detailedmeasure','results');
        else
            save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_feeder_sansC1lowelec_detailedmeasure','results');
        end
    else
        if(isFeederJumperOpen == true)
            if(isC1low1Active == true)
                save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_6Vext_avecC1low_jumperopen_detailedmeasure','results');
            else
                save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_R'+round(R)+'Ohms_6Vext_sansC1lowelec_jumperopen_CVB_detailedmeasure','results');
            end
        else
            if(isC1low1Active == true)
                save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_6Vext_avecC1low_detailedmeasure','results');
            else
                save('sim_results/20260216/results_phase_circuit1_VDC'+VDC_name+'V_freq'+round(f0)+'Hz_Larm'+round(L_arm*1e6)+'uH_Cm'+round(C_m*1e6)+'uF_6Vext_sansC1lowelec_detailedmeasure','results');
            end
        end
    end
end
